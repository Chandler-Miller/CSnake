...
// struct Snake* snake, struct Food* food, struct PathNode* start, struct
// PathNode* dest, int grid_rows, int grid_cols
...
struct PathNode **a_star_search(struct PathNode *start, struct PathNode *dest,
				int **grid, int grid_rows, int grid_cols,
				int *length)
{
	...
	while (open_list_length > 0) {
		...
		for (int i = 1; i < open_list_length; i++) {
			if (open_list[i]->G + open_list[i]->H <
			    current->G + current->H) {
				current = open_list[i];
				current_index = i;
			}
		...
		for (int i = 0; i < num_neighbors; i++) {
			...
			double newG =
			    current->G + sqrt(pow(neighbor->X - current->X, 2) +
					      pow(neighbor->Y - current->Y, 2));
			...
			if (!in_open_list || newG < neighbor->G) {
				...
				neighbor->G = newG;
				neighbor->H = sqrt(pow(neighbor->X - dest->X, 2) +
						   pow(neighbor->Y - dest->Y, 2));
...
void update_snake(struct Snake *snake, struct Food *food)
{
	...
	while (current != NULL) {
		...
		current->x_direction =
		    (prev != NULL) ? prev->X - current->X : snake->x_direction;
		current->y_direction =
		    (prev != NULL) ? prev->Y - current->Y : snake->y_direction;
	...
	if (snake->parts->X < 0 || snake->parts->X >= width ||
	    snake->parts->Y <= 1 || snake->parts->Y >= height) {
		gameover = true;
		return;
	}
...
void draw(struct Node *snake, struct Food *food, struct PathNode *start,
	  struct PathNode *dest, int **grid)
{
	clear();
...
void gameloop(struct Snake *snake, struct Food *food, struct PathNode *start,
	      struct PathNode *dest, int grid_rows, int grid_cols)
{
	int input = 0;
	int grid[30][60] = {0};
	...
	while (!gameover) {
		...
		struct PathNode **path =
		    a_star_search(start, dest, grid, grid_rows, grid_cols, &length);
...
int main()
{
	...
	struct PathNode start = {
	    .X = head->X, .Y = head->Y, .G = 0.0, .H = 0.0, .Parent = NULL};
	// malloc(sizeof(struct PathNode*));
	// start->X = head->X;
	// start->Y = head->Y;
	// start->G = 0.0;
